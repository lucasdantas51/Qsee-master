  ...
  <div v-cloak v-if="$store.state.user != 'logged-out'" id="cases">
    <nav class="breadcrumb" aria-label="breadcrumbs">
      <ul>
        <li>
          ...
          <nuxt-link to="/">Inicial</nuxt-link>
        </li>
        <li class="is-active">
    ...
    <div v-if="withCustomer > 0" class="section has-text-centered">
      ...
      <h2 class="title is-4">Ticket {{withCustomer}} ({{withCustomerTopic}})</h2>
       -->
      <h1 class="title is-2">Você tem um cliente</h1> <!-- Voce tem um atendimento -->
      <h2 class="title is-4">Chamado {{withCustomer}} ({{withCustomerTopic}})</h2>
      <nav class="level">
        <div class="level-item has-text-centered">
          <!-- <button @click="callAgain" class="button is-danger is-large":class="{'is-loading': called}">Call customer again</button> -->
          <button @click="callAgain" class="button is-danger is-large" :class="{'is-loading': called}">Ligar novamente</button>
        </div>
        <div class="level-item has-text-centered">
          <!-- <button @click="newCustomer" class="button is-primary is-large">Take another customer</button> -->
          <button @click="newCustomer" class="button is-primary is-large">Outro atendimento</button>
        </div>
      </nav>
    </div>
    ...
    <div v-if="withCustomer == 0" class="columns">
      ...
      <div id="queue" class="column has-text-centered">
        <div class="box">
          <div>
            <span id="queueTotal">
              <!-- <h3 class="title is-4">Queue ({{waiting.length}} waiting | {{atDesks.length}} at desks)</h3> -->
              <h3 class="title is-4">Fila ({{waiting.length}} em espera | {{atDesks.length}} em atendimento)</h3>
            </span>
          </div>
          ...
          <div id="summaryBox">
            ...
            <h3 v-if="waiting.length" class="title is-5 has-text-primary">Em espera</h3>
            ...
            <br>
            <h3 v-if="atDesks.length" class="title is-5 has-text-primary">Em atendimento</h3>
