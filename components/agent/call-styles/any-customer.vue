<template>
  <div class="box">
    <h2 class="title is-4 has-text-centered">Escolha uma pessoa em espera</h2>
    <p
    ...
    >
      <button @click="takeCustomer(tkt.id, tkt.printed)" class="button is-primary">
        <span>Chamado {{tkt.id}} ({{tkt.topic}}) {{time(tkt.printed)}}</span>
      </button>
      <br>
      <br>
    </p>
    <p v-if="!waitingCustomers.length" class="has-text-centered">Nenhuma pessoa esperando</p>
  </div>
</template>
