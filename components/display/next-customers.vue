<template>
  <div v-if="next.length" class="box" style="width:100%">
    <span v-if="settings.takeCustomer == 1">
      <h1 v-for="tkt in nextNonPay" :key="tkt.id" class="title is-1 spacing">Próximo chamado: {{tkt.id}}</h1>
      <h1 v-for="tkt in nextPay" :key="tkt.id" class="title is-1 spacing">Próximo pagamento: {{tkt.id}}</h1>
    </span>
    <h1
